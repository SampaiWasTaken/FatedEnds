...
func _on_area_3d_body_exited(body: Node3D) -> void:
	if not corpse_open and (body.is_in_group ("player") or body.is_in_group("corpse") or body.is_in_group("crate")):
...
func _on_area_3d_body_entered(body: Node3D) -> void:
	print(body)
	if not corpse_open:
		if body.is_in_group("player") or body.is_in_group("crate"):
